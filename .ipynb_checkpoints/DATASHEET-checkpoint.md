...
*   **How many instances are there?**
    The dataset contains **176 total data points** (as of Round 13), distributed as:
    *   **8 Functions** independent of each other.
    *   **22 evaluations per function** (10 initial random points + 12 weekly strategic queries).

*   **What is the format of the data?**
    *   **Master File:** `bbo_master_w13.csv` (Aggregated CSV containing all history).
    *   **Weekly Queries:** JSON files (e.g., `week13_clean_inputs.json`) containing specific weekly submissions.

*   **Does the data set contain all possible instances or is it a sample?**
    It is a highly sparse sample. The search space is continuous, discretised to 6 decimal places. With only 22 points covering an 8-dimensional hypercube (Function 8), the data is extremely sparse, leaving vast regions of the search space unexplored.
...
*   **How was the data associated with each instance acquired?**
    Data was acquired iteratively over a 13-week period. Each week, a machine learning strategy proposed new $X$ coordinates, which were submitted to the capstone portal to retrieve the corresponding $Y$ values.
...
*   **What mechanisms or strategies were used to collect the data?**
    ...
    *   **Week 10 (Posterior Refinement):** Reverted to **Bayesian Optimisation (Expected Improvement)** to capitalise on the variance reduction from Week 9, using a Matern 2.5 + WhiteKernel to handle observation noise.
    *   **Week 11 (Cluster-Based Trust Regions):** Implement **Local Penalisation** by restricting the optimiser to a hypersphere (Trust Region) of radius R centred on the best historical point (x_best), forcing the model to exploit specific high-performing clusters rather than searching globally.
    *   **Week 12 (Dimensionality Reduction):** Applied a **Principal Component Sensitivity** strategy. For high-dimensional functions (F6–F8), low-sensitivity input dimensions were "frozen" at their best-known values, focusing the query budget solely on the dimensions driving the highest output variance.
    *   **Week 13 (Experience Replay):** Final submission utilised a deterministic **Experience Replay** policy, re-submitting the historical parameters that yield the maximum observed Y to verify results against stochastic noise without model influence.

*   **Over what timeframe was the data collected?**
    Data was collected using an iterative Python loop. Weeks 1–12 utilised a Gaussian Process Regressor (Matern 2.5 + WhiteKernel) with acquisition functions (UCB/EI).
